# -*- coding: utf-8 -*-
"""
Created on Thu Jan 11 18:16:52 2018
...
def inference(images_placeholder, keep_prob):
    ...
    cross_entropy = -tf.reduce_sum(y_conv*tf.log(y_conv))
    return y_conv
...
face_img = faceDetect(images)
if len(face_img) > 0:
    placeholder = []
    #cv2.imshow("1",face_img)
    #cv2.waitKey(0)
    placeholder.append(face_img.flatten().astype(np.float32)/255.0)
    ...
    pred = np.argmax(logits.eval(session=sess,feed_dict={images_placeholder: placeholder,keep_prob: 1.0 }))
    print((np.amax(logits.eval(session=sess,feed_dict={images_placeholder: placeholder,keep_prob: 1.0 })))*100)
    print('Test Label: %s' %pred)
else:
